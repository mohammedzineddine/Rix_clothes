...
// Helper function to process options arrays from database
...
function addToCart(product, options = {}) {
    ...
    if (existingIndex >= 0) {
        ...
        cart[existingIndex].qty += (options.qty || 1);
    } else {
        cart.push({
            id: product.id,
            name: product.name,
    ...
    const sizeText = options.size ? `Size: ${options.size}` : '';
    const colorText = options.color ? `Color: ${options.color}` : '';
    const optionsText = [sizeText, colorText].filter(Boolean).join(', ');
    alert(`Added ${product.name}${optionsText ? ` (${optionsText})` : ''} to cart!`);
}
...
async function generateSampleProducts() {
    const sampleProducts = [
        ...
        },
        {
            ...
            available_colors: ['White', 'Blue', 'Navy'],
        ...
        },
        {
            ...
            available_sizes: ['XS', 'S', 'M', 'L'],
            available_colors: ['Black', 'Navy', 'Burgundy'],
        ...
        },
        {
            ...
            available_colors: ['Black', 'Gray', 'Camel'],
            materials: '100% Wool',
            stock_status: 'in_stock',
            sku: 'OC-004'
        },
